  ...
  endpoints: (builder) => ({
    getFeed: builder.query<any[], void>({
      ...
      async onCacheEntryAdded(arg, { updateCachedData, cacheDataLoaded, cacheEntryRemoved, dispatch }) {
        const socket = io(process.env.NEXT_PUBLIC_SOCIAL_SOCKET_URL || 'http://localhost:3007');
    ...
    // Friends
    sendFriendRequest: builder.mutation<any, { requester: string; requesterName: string; recipient: string; recipientName: string }>({
      query: (body) => ({
